...
export class SalesforceCRM extends CRMProvider {
	private config: SalesforceConfig;

	constructor(instanceUrl: string, clientId: string, clientSecret: string) {
		super();
		this.config = { instanceUrl, clientId, clientSecret };
	}

	private async getAccessToken(): Promise<string> {
		const response: Response = await fetch(`${this.config.instanceUrl}/services/oauth2/token`, {
			method: "POST",
	...
	async fetchCompanies(): Promise<StandardizedCRMCompany[]> {
		...
		const accountsRes: Response = await fetch(
			`${this.config.instanceUrl}/services/data/v65.0/query?q=${encodeURIComponent(accountsQuery)}`,
			{
				headers: {
		...
		const casesRes: Response = await fetch(
			`${this.config.instanceUrl}/services/data/v65.0/query?q=${encodeURIComponent(casesQuery)}`,
		...
		const oppRes: Response = await fetch(
			`${this.config.instanceUrl}/services/data/v65.0/query?q=${encodeURIComponent(oppQuery)}`,
			{ headers: { Authorization: `Bearer ${accessToken}` } },
		);
		...
		const taskRes: Response = await fetch(
			`${this.config.instanceUrl}/services/data/v65.0/query?q=${encodeURIComponent(taskQuery)}`,
			{ headers: { Authorization: `Bearer ${accessToken}` } },
		);
