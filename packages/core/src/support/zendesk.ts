...
export class ZendeskSupport extends SupportProvider {
	...
	private async getAccessToken(): Promise<string> {
		const response: Response = await fetch(
			`https://${this.config.subdomain}.zendesk.com/oauth/tokens`,
			{
				method: "POST",
	...
	async fetchCustomers(): Promise<StandardizedSupportCustomer[]> {
		...
		const ticketsRes: Response = await fetch(
			`https://${this.config.subdomain}.zendesk.com/api/v2/tickets.json`,
			{
				headers: {
		...
		const orgsRes: Response = await fetch(
			`https://${this.config.subdomain}.zendesk.com/api/v2/organizations.json`,
	...
	authorize(clientId: string, redirectUri: string, userId: string, subdomain?: string): string {
		const state: string = btoa(JSON.stringify({ userId, provider: "zendesk", subdomain: subdomain || this.config.subdomain }));
		...
		return `https://${subdomain || this.config.subdomain}.zendesk.com/oauth/authorizations/new?${params}`;
	}

	async callback(code: string, clientId: string, clientSecret: string, redirectUri: string, subdomain?: string): Promise<{ subdomain: string; clientId: string; clientSecret: string }> {
		const tokenResponse: Response = await fetch(
			`https://${subdomain || this.config.subdomain}.zendesk.com/oauth/tokens`,
			{
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({
